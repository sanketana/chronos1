...
export async function POST(req: NextRequest) {
    ...
    const result = await client.query('SELECT id, password FROM users WHERE email = $1 AND role = $2', [email, 'student']);
    await client.end();
    ...
    const res = NextResponse.redirect(new URL('/dashboard/students', req.nextUrl.origin), { status: 302 });
